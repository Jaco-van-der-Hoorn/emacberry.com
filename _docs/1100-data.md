...
By default, an application has a certain amount of screens (called GUI) and the code is running when the so called
_Activity_ is visible for the user. Even if GPSLogger also has such a GUI part [as mainly described here](./1200-views.html)
...
Starting a Background Service on Android requires that the code running will always show the user a (sticky)
...
Battery lives matter - that's one of the core principles that drives me, during the development of the application. As
...
> Just as one example: GPSLogger doesn't use an internal database while logging, instead the data will be directly written
on your device. This makes the code a bit more complex, but on the other hand this saves a lot of CPU-cycles (and
...
in magnetic sensor (aka compass) of my current Pixel3 phone has an almost insane negative impact on the overall battery
...
And without going into too much into details here - since you might like to use different update-intervals for different use
cases the app offers a [Multi Preferences/Modes Feature](../3950-multipref) allowing you to quickly & easily adjust the
settings. 
...
1. All calculations/manipulations will happen after the **raw source data** has been captured and stored inside the GPL
   file. So all optimizations will always happen afterwards - the original data will never be altered/modified!<br/><br/>
   This ensures that when ever something will be messed up inside the application (e.g. cause of a bug) it can
   **always** be fixed with an update of the GPSLogger application!

2. All possible data post-processing can be disabled via the application setting (or needs to be enabled by the user in
   the first place). IMHO it's essential that the user can decide **if** and then **how** the sensor data should be
   manipulated. GPSLogger is **no BLACKBOX<span class="material-icons">help_center</span>** nor does it do any
...
the data/fields for each supported sensor as a separate/different _record-type_. By following this approach, it's quite
...
Even if most (if not all) website location based processing information have a minimum resolution of **one second**
...
> computers) have the same one-second-resolution (probably because of hardware limitations).
>
> This resolution is also present in some quite popular file formats, like
> [Garmin's FIT format](https://developer.garmin.com/fit) or TCX (where the fields of the type _dateTime_ have also only
...
> pushing the pedals with a cadence greater than 60 revs per minute. In other words, in one second I do more than a
> single rotation of the crank. When I additionally process data from a power-meter (pedals) then it's IMHO essential to
> know in which fraction of a second a certain power value was provided - especially when reading power-meter data from
> left and right pedals individually (which I actually do).
>
...
able to provide such location objects (despite this the app can request to receive the location in a lower frequency then the
OS is able to provide the data).

I have said that - especially when you are **indoors**, the chances that your GPS signal is **quite bad**, is **quite high**
...
not using them on purpose**!

The following information is stored everytime a new **gps based** location has become available to the app:
- Timestamp (in milliseconds) when the location was received on the device \[_Please note that the location object
  itself contains also time information - this internal time information will be ignored!_\]
- Latitude
- Longitude
...
> Satellites_. Obviously no accuracy or any additional NMEA-data will be available for these types of simulated activities. 
...
You caught me - **I am a liar**! I have said, that all sensor data will be recorded independently of each other? I am
sorry - for historic reasons this statement is not correct for the magnetic sensor data.

When new GPS location information arrives, the original course value withing the _gps location information_ will be
...
[Please read **Using Barometer based Elevation - Theory of operation** for further details of why recording pressure
...
Initially the app stores the configured number of front & rear gear teeth in a path (so when you change this gear setup
...
- Resulting gear ratio (only because of historic reasons - this data is actually obsolete)
...
- Timestamp (in milliseconds)
- _too much_ (_will be done when I have more time to do this_)

### <span class='material-icons fa-fw' style='width: 1.042em;'>rowing</span> Concept2 Rower Data
- Timestamp (in milliseconds)
- _way too much_ (_will be done when I have way more time to do this_)

# How to access / export the data
## The GPS file format
IF I am ever in the right mode, I am going to release details in GitHub. For earlier file versions there exists an
external reading code - but since the changes introduced with the V6 implementation nobody seams to be interested in an
...
[^1]: WGS84 ellipsoid elevation information might not match the _Mean sea level_ (MSL) elevation information you expect
      as a user! If you are interested in the details & differences of WGS84 compared to the natural elevation you might
...
[^2]: The _GeoIdHeight_ is the delta between the WGS84 elevation and the _Mean sea level_ (MSL) height (based on
      EGM2008) - This value will be extracted from the plain NMEA-sentences data (since is not provided by OS location
      objects directly). The good news is, even if the value is dependant from your current location, that the
      GeoIdHeight does not change that frequently.
      ...
      previous recorded paths do not contain a GeoIdHeight value in the location recond. In such a case the app is
      able to calculate the GeoIdHeight value by itself. This is quite some intense maths-stuff and with that, expect
